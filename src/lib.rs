...
use rayon::prelude::*;
use pyo3::prelude::*;
use numpy::{self, PyArray2, PyReadonlyArray2};
use numpy::ndarray::{Array, Array1, Array2, Array3, ArrayView, ArrayView1, ArrayView2, ArrayView3, ArrayViewD, };
...
#[pymodule]
fn tgv_denoise_image<'py>(_py: Python<'py>, m: &Bound<'py, PyModule>) -> PyResult<()> {

    #[pyfn(m)]
    #[pyo3(name = "tgv_denoise")]
    fn tgv_denoise_py<'py>(py: Python<'py>, u0: PyReadonlyArray2<f32>, lam: f32, alpha0: f32, alpha1: f32, tau: f32, sigma: f32, n_iter: i32) -> Bound<'py, PyArray2<f32>> {
        let u0 = u0.as_array();
        let out = tgv_denoise(&u0, lam, alpha0, alpha1, tau, sigma, n_iter);
        out.into_pyarray(py)
    }

    #[pyfn(m)]
    #[pyo3(name = "parallel_tgv_denoise")]
    fn parallel_tgv_denoise_py<'py>(py: Python<'py>, u0: PyReadonlyArray2<f32>, lam: f32, alpha0: f32, alpha1: f32, tau: f32, sigma: f32, n_iter: i32) -> Bound<'py, PyArray2<f32>> {
        let u0 = u0.as_array();
        let out = parallel_tgv_denoise(&u0, lam, alpha0, alpha1, tau, sigma, n_iter);
        out.into_pyarray(py)
    }
